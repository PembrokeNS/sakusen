...
LIBUNICODE_API void unicode_init (void);

/* Returns 1 if current locale uses UTF-8 charset.  If CHARSET is
   not null, sets *CHARSET to the name of the current locale's
   charset.  This value is statically allocated.  */
LIBUNICODE_API int unicode_get_charset (char **charset);

/* These are all analogs of the <ctype.h> functions.  */
LIBUNICODE_API int unicode_isalnum (unicode_char_t c);
LIBUNICODE_API int unicode_isalpha (unicode_char_t c);
LIBUNICODE_API int unicode_iscntrl (unicode_char_t c);
LIBUNICODE_API int unicode_isdigit (unicode_char_t c);
LIBUNICODE_API int unicode_isgraph (unicode_char_t c);
LIBUNICODE_API int unicode_islower (unicode_char_t c);
LIBUNICODE_API int unicode_isprint (unicode_char_t c);
LIBUNICODE_API int unicode_ispunct (unicode_char_t c);
LIBUNICODE_API int unicode_isspace (unicode_char_t c);
LIBUNICODE_API int unicode_isupper (unicode_char_t c);
LIBUNICODE_API int unicode_isxdigit (unicode_char_t c);
LIBUNICODE_API int unicode_istitle (unicode_char_t c);
LIBUNICODE_API int unicode_isdefined (unicode_char_t c);
LIBUNICODE_API int unicode_iswide (unicode_char_t c);

/* More <ctype.h> functions.  These convert between the three cases.
   See the Unicode book to understand title case.  */
LIBUNICODE_API unicode_char_t unicode_toupper (unicode_char_t c);
LIBUNICODE_API unicode_char_t unicode_tolower (unicode_char_t c);
LIBUNICODE_API unicode_char_t unicode_totitle (unicode_char_t c);

/* If C is a digit (according to `unicode_isdigit'), then return its
   numeric value.  Otherwise return -1.  */
LIBUNICODE_API int unicode_digit_value (unicode_char_t c);

/* If C is a hex digit (according to `unicode_isxdigit'), then return
   its numeric value.  Otherwise return -1.  */
LIBUNICODE_API int unicode_xdigit_value (unicode_char_t c);

/* Return the Unicode character type of a given character.  */
LIBUNICODE_API int unicode_type (unicode_char_t c);
...
LIBUNICODE_API char *unicode_previous_utf8 (const char *start, const char *p);
...
LIBUNICODE_API char *unicode_next_utf8 (const char *p);
...
LIBUNICODE_API int unicode_strlen (const char *p, int max);

/* Returns the visual width, in character-size units, of P, a string.
   This value may be used for tabulation.  */
LIBUNICODE_API int unicode_string_width (const char *p);
...
LIBUNICODE_API char *unicode_get_utf8 (const char *p, unicode_char_t *result);

/* Returns the offset within the string, in bytes, of the character offset
   given. */

LIBUNICODE_API size_t unicode_offset_to_index(const char *p, int offset);

/* Returns the offset within the string, in characters, of the byte offset
   given. */

LIBUNICODE_API size_t unicode_index_to_offset(const char *p, int offset);

/* Returns a pointer to the _last_ non-NULL utf-8 within the string */
	
LIBUNICODE_API char *unicode_last_utf8(const char *p);

/* Copies n characters from src to dest */

LIBUNICODE_API char *unicode_strncpy(char *dest, const char *src, size_t n);

/* Find the UTF-8 character corresponding to ch, in string p. These
   functions are equivilants to strchr and strrchr */

LIBUNICODE_API char *unicode_strchr(const char *p, unicode_char_t ch);
LIBUNICODE_API char *unicode_strrchr(const char *p, unicode_char_t ch);

/* Pads a string to fill out a requested visual width */

LIBUNICODE_API void unicode_pad_string(char *dest, int right, int width, const char *string);
	
/* Compute canonical ordering of a string in-place.  This rearranges
   decomposed characters in the string according to their combining
   classes.  See the Unicode manual for more information.  */
LIBUNICODE_API void unicode_canonical_ordering (unicode_char_t *string, size_t len);

/* Compute canonical decomposition of a character.  Returns malloc()d
   string of Unicode characters.  RESULT_LEN is set to the resulting
   length of the string.  */
LIBUNICODE_API unicode_char_t *unicode_canonical_decomposition (unicode_char_t ch,
						 size_t *result_len);
...
/* Create a new iconv conversion instance.  TOCODE is the destination
   ...
   ENOMEM or EINVAL.  */
LIBUNICODE_API unicode_iconv_t unicode_iconv_open (const char *tocode, const char *fromcode);

/* Close an iconv conversion instance.  */
LIBUNICODE_API int unicode_iconv_close (unicode_iconv_t cd);
...
LIBUNICODE_API ssize_t unicode_iconv (unicode_iconv_t cd,
